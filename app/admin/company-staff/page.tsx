...
export default function CompanyStaffPage() {
  ...
  useEffect(() => {
    ...
    // Dùng History API thay vì router.replace để không gây re-render hay Suspense
  ...
  const handleAddEmployee = async () => {
    ...
    try {
      setAddLoading(true);
      const monthlySalary = addForm.monthlySalary
        ? Number(parseFormattedNumber(addForm.monthlySalary))
        : 0;
      const allowance = addForm.allowance
        ? Number(parseFormattedNumber(addForm.allowance))
        : 0;
      const socialInsurance = addForm.socialInsurance
        ? Number(parseFormattedNumber(addForm.socialInsurance))
        : 0;
      const monthlyAdvanceLimit = addForm.monthlyAdvanceLimit
        ? Number(parseFormattedNumber(addForm.monthlyAdvanceLimit))
        : 0;
      ...
      const response = await employeeService.createCompanyStaff(payload);
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            setPageSize(ps);
                            setCurrentPage(0);
                            // immediately fetch with new pageSize and page 0 to avoid stale request
                      ...
                      {cccdFrontPreview ? (
                        ...
                        <>
                          <input
                            type="file"
                            accept="image/*"
                            id="cs-cccd-front-input"
                            onChange={handleCccdFrontChange}
                            className="hidden"
                          />
                          <label
                            htmlFor="cs-cccd-front-input"
                      ...
                      {cccdBackPreview ? (
                        ...
                        <>
                          <input
                            type="file"
                            accept="image/*"
                            id="cs-cccd-back-input"
                            onChange={handleCccdBackChange}
                            className="hidden"
                          />
                          <label
                            htmlFor="cs-cccd-back-input"
                            className="cursor-pointer flex flex-col items-center justify-center py-4"
                          >
                            <svg
                              className="w-7 h-7 text-blue-500 mb-1"
