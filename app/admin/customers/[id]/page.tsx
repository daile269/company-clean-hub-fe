      ...
      <div className="mt-6 bg-white rounded-lg shadow-md p-6">
        <div className="flex items-center justify-between mb-4 pb-2 border-b">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={() => {
                // default select first contract/assignment if available
                const firstContract = contracts[0];
                const allAssignments = allAssignedEmployees.flatMap(
                  (g: any) => g.assignments || []
                );
                const firstAssignment = allAssignments[0];
                setReviewForm({
                  contractId: firstContract ? String(firstContract.id) : "",
                  assignmentId: firstAssignment
                    ? String(firstAssignment.id)
                    : "",
                  employeeId: firstAssignment
                    ? String(
                        firstAssignment.employeeId ?? firstAssignment.employeeId
                      )
                    : "",
                  rating: 5,
                  comment: "",
        ...
        <div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 max-w-md w-full shadow-2xl">
            ...
            <div className="space-y-3">
              <div>
                ...
                >
                  ...
                  {contracts.map((c: any) => (
                    <option key={c.id} value={c.id}>{`HĐ #${c.id} - ${
                      c.description ||
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    // find assignment from allAssignedEmployees
                    const all = allAssignedEmployees.flatMap(
                      (g: any) => g.assignments || []
                    );
                    const sel = all.find((a: any) => String(a.id) === aid);
                    setReviewForm({
                      ...reviewForm,
                      assignmentId: aid,
                      employeeId: sel
                ...
                >
                  ...
                  {allAssignedEmployees
                    .flatMap((g: any) => g.assignments || [])
                    .map((a: any) => (
                      <option key={a.id} value={a.id}>{`${
                        a.employeeName || a.name || `ID:${a.employeeId}`
                      } ${a.employeeCode ? `(${a.employeeCode})` : ""} - HĐ#${
                        a.contractId ?? ""
                      }`}</option>
                    ))}
                </select>
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button
                onClick={async () => {
                  if (!id) return toast.error("Không có mã khách hàng");
                  ...
                  try {
                    ...
                    if (res && res.success) {
                      ...
                      try {
                        ...
                        setCustomerReviews(refreshed.content || []);
                      } catch (err) {
                        console.error(
                          "Error refreshing customer reviews after add:",
                          err
                        );
                      } finally {
                        setLoadingCustomerReviews(false);
